"""
Optimized Semi-Vector MCCFR for Leduc Poker.

Simpler optimizations:
1. Pre-compute tree structure as arrays (no recursion)
2. Use integer keys instead of strings
3. Cache infoset lookups
4. Vectorize strategy computation

This should give 5-10x speedup while being correct.
"""
...
class SemiVectorLeducOpt:
    """Optimized Semi-Vector MCCFR for Leduc."""
    ...
    def _build_tree(self):
        ...
        # Enumerate all action sequences for both rounds
        self.r1_nodes = []
        self.r2_nodes = []
            ...
            for comm in boards:
                ...
                for player in range(2):
                    self._update_regrets(
                        ...
                        strat, deal_idx, weight
                    )

        self._cumulative_strategy += t * strat

    def _update_regrets(self, hero: int, villain: int, comm: int,
                        player: int, strat: np.ndarray, deal_idx: int, weight: float):
        """Update regrets using iterative tree traversal."""
        ...
        # Process round 1 (backwards)
        r1_values = {}  # action_id -> (p0_ev, p1_ev)
        ...
        for node in reversed(self.r1_nodes):
            ...
            if node['type'] == 'terminal':
                ...
                if key in self.infoset_key_to_idx:
                    ...
                    # Update regrets if this is player's infoset
                    if acting_player == player and acting_rank == player_rank:
                        for a_idx in range(n_actions):
                            av = child_values[a_idx][player]
                            ev = r1_values[aid][player]
                            self._cumulative_regret[h_idx, a_idx, deal_idx] += weight * (av - ev)
                else:
                    r1_values[aid] = (0.0, 0.0)

    def _compute_r2(self, hero_rank, villain_rank, comm_rank,
                    ...
                    for p in range(2):
                        if acting_player == p and acting_rank == player_rank[p]:
                            for a_idx in range(n_actions):
                                ...
                                ev = r2_values[aid][p]
                                # Use smaller weight for r2
                                self._cumulative_regret[h_idx, a_idx, deal_idx] += (1.0/30/4) * (av - ev)
                else:
                    r2_values[aid] = (0.0, 0.0)
